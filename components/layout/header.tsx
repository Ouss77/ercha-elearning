  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border/50 bg-card/80 backdrop-blur-md shadow-sm">
      <div className="flex h-16 items-center justify-between gap-2 px-2 sm:px-4 lg:px-6">
        {/* Mobile menu button */}
        <Button variant="ghost" size="icon" className="lg:hidden hover:bg-primary/10 flex-shrink-0" onClick={onMenuClick}>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 lg:hidden flex-1 min-w-0">
          <Link href="/">
            ...
            <span className="text-base sm:text-lg font-bold bg-gradient-to-r from-foreground to-foreground/70 bg-clip-text text-transparent truncate">EduPlatform</span>
          </Link>
        </div>
        ...
        <div className="flex items-center gap-1 sm:gap-2 lg:gap-4 flex-shrink-0">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <Avatar className="h-8 w-8 sm:h-9 sm:w-9 ring-2 ring-primary/20 flex-shrink-0">
              ...
              <AvatarFallback className="bg-gradient-to-br from-primary/20 to-chart-2/20 text-primary font-semibold text-xs sm:text-sm">{initials}</AvatarFallback>
            </Avatar>
            <div className="hidden md:flex md:flex-col md:items-start min-w-0">
              <span className="text-sm font-semibold text-foreground truncate max-w-[120px] lg:max-w-[200px]">{user.name || user.email}</span>
              {user.role && <Badge className={`text-xs border whitespace-nowrap ${getRoleBadgeClass(user.role)}`}>{roleLabels[user.role]}</Badge>}
            </div>
          </div>
          <LogoutButton />
        </div>
